...
class Store {
	constructor() {
		...
		this.undoCount = 0;
		this.color = 'black';
		this.backUpHistory = [this.data.shapes]
	...
	undo() {
		...
		if (this.data.shapes.size >= 1) {
			this.undoCount = this.undoCount + 1
			let selectedPosition = this.history.length - this.undoCount
			this.data.shapes = this.history[selectedPosition - 1]
			this.emitChanges();
		}
	}
